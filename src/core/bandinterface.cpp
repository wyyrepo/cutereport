...
void BandInterface::setGeometry(const QRectF & rect, Unit unit)
{
    ...
    if (d->geometry  == newRect)
        return;

    d->geometry = newRect;

    if (d->geometry.width() < d->minRectSize.width())
        d->geometry.setWidth(d->minRectSize.width());
    if (d->geometry.height() < d->minRectSize.height())
        d->geometry.setHeight(d->minRectSize.height());
...
void BandInterface::setMargins(const CuteReport::Margins &margins)
{
    Q_D(BandInterface);
...
bool BandInterface::renderNewPageComplete()
{
...
bool BandInterface::adjustSize()
{
    ...
    if (d->stretchable && d->showStretchability) {
        QSizeF size = d->stretched ? d->stretchOriginalSize : d->geometry.size();
        ...
        if (size.height() > baseSize.height()) {
            if (!d->stretched) {
                d->stretched = true;
                d->stretchOriginalSize = d->geometry.size();
            }
        ...
        if (d->stretched) {
            d->stretched = false;
            if (d->geometry.size() != d->stretchOriginalSize) {
                d->geometry.setSize(d->stretchOriginalSize );
                result = true;
            }
        }
    }
