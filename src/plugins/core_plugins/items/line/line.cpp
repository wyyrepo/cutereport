    ...
    :ItemInterface(new LineItemPrivate, parent)
{
    ...
    d->geometry = QRectF(0,0,100,8);
    d->frame = 0;
}


bool LineItem::moduleInit()
{
    initMyResource();
    return true;
}
...
CuteReport::BaseItemHelperInterface * LineItem::createHelper()
{
    return 0;
}
...
bool LineItem::renderBegin()
{
    rendererBeginBaseStart(new LineItemPrivate(*(reinterpret_cast<LineItemPrivate*>(d_ptr))));
...
RenderedItemInterface *LineItem::renderView(quint32 id)
{
    Q_D(LineItem);
...
void LineItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
    ...
    painter->setPen(penPrepared(d->linePen, d->dpi));
...
void LineItem::renderInit(CuteReport::ScriptEngineInterface *scriptEngine, CuteReport::InitFlags flags)
{
...
Q_EXPORT_PLUGIN2(LineItem, LineItem)
#endif
