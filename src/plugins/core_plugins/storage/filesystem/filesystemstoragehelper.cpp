...
FileSystemStorageHelper::FileSystemStorageHelper(StorageFileSystem *storage, CuteDesigner::Core *designer, VisibleOptions visibleOptions) :
...
void FileSystemStorageHelper::load()
{
    ...
    if (m_storage->rootRelative() != ui->cbRootRelative->isChecked())
        ui->cbRootRelative->setChecked(m_storage->rootRelative());
