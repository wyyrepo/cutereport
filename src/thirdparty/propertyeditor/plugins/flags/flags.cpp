...
void Flags::buttonPressed()
{
	...
	{
        int val = object()->metaObject()->enumerator(object()->metaObject()->indexOfEnumerator(object()->metaObject()->property(objectProperty()).typeName())).value(en);
        ...
        f.list->item(en)->setCheckState(((value().toInt() & val) == val ) ? Qt::Checked : Qt::Unchecked);
	}
