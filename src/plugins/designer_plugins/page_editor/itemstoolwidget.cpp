...
void ItemsToolWidget::addItem(const QIcon & icon, const QString & name, const QString &suiteName, const QString & group)
{
    ...
    if (group.isEmpty()) {
        ...
        QString moduleName = suiteName.isEmpty() ? name : suiteName + "::" + name;
        newItem->setData(moduleName);
...
void Menu::mousePressEvent ( QMouseEvent * event )
{
    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        mimeData->setText(QString("CuteReport_BaseItemInterface::%1").arg(action->data().toString()));
