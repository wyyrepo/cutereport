...
ResourceStorageHelper::ResourceStorageHelper(StorageResource *storage, CuteDesigner::Core *designer, VisibleOptions visibleOptions) :
    StorageHelperInterface(storage, designer),
...
//    QString selectedObjectUrl = d.currentObjectUrl();
    ...
    QString selectedObjectUrl = m_designer->loadObjectDialog();
...
void ResourceStorageHelper::deleteObject()
{
    if (!ui->objectPrefixList->currentItem())
        return;
    /// root object dont have any prefix
    QString fullObjectName = (m_currentObjectPrefix == "/" ? "" : m_currentObjectPrefix) +
                             (m_currentObjectPrefix.endsWith('/') ? "" : "/") +
                             m_currentObjectName;
    m_storage->objects().remove(fullObjectName);
    fillObjects(m_currentObjectName);
    updatePreview();
}
...
//    m_currentReport = d.currentObjectUrl();
    m_currentReport = m_designer->loadReportDialog();
