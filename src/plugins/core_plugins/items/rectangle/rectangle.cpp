    ...
    :ItemInterface(new RectangleItemPrivate, parent)
{
    ...
    d->geometry = QRectF(0,0,40,20);
}


bool RectangleItem::moduleInit()
{
    initMyResource();
    return true;
}
...
CuteReport::BaseItemHelperInterface *RectangleItem::createHelper()
{
    return 0;
}
...
bool RectangleItem::renderBegin()
{
    rendererBeginBaseStart(new RectangleItemPrivate(*(reinterpret_cast<RectangleItemPrivate*>(d_ptr))));
...
RenderedItemInterface *RectangleItem::renderView(quint32 id)
{
    Q_D(RectangleItem);
...
void RectangleItem::renderInit(CuteReport::ScriptEngineInterface *scriptEngine, CuteReport::InitFlags flags)
{
...
Q_EXPORT_PLUGIN2(RectangleItem, RectangleItem)
#endif
