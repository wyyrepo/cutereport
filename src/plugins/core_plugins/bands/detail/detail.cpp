...
void Detail::renderInit(ScriptEngineInterface *scriptEngine, InitFlags flags)
{
    Q_D(Detail);
...
bool Detail::renderBegin()
{
    rendererBeginBaseStart(new DetailPrivate(*(reinterpret_cast<DetailPrivate*>(d_ptr))));
    ...
    if (d->alternateRow) {
        if (d->alternateBrush == QBrush())
            d->backgroundBrush.setColor(d->backgroundBrush.color().darker(110));
        else
            d->backgroundBrush = d->alternateBrush;
    }
...
RenderedItemInterface * Detail::renderView(quint32 id)
{
    Q_D(Detail);
    CuteReport::RenderedItemInterface * view = new DetailRenderedItem(this, new DetailPrivate(*d));
    return view;
}
...
void Detail::setAllowSplit(bool value)
{
    Q_D(Detail);
    if (d->allowSplit == value)
        return;
    d->allowSplit = value;
    emit allowSplitChanged(d->allowSplit);
    emit changed();
}


bool Detail::isSplitAllowed() const
{
    Q_D(const Detail);
    return d->allowSplit;
}
...
QPair<QString, int> Detail::sortingOrder() const
{
    return QPair<QString, int>("detail", 500);
}
