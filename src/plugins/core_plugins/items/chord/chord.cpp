...
BaseItemHelperInterface *ChordItem::createHelper(DesignerItemInterface *designer)
{
    Q_UNUSED(designer)
    return 0;
}
...
bool ChordItem::renderBegin()
{
    rendererBeginBaseStart(new ChordItemPrivate(*(reinterpret_cast<ChordItemPrivate*>(d_ptr))));
...
RenderedItemInterface *ChordItem::renderView(quint32 id)
{
    Q_D(ChordItem);
...
void ChordItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
...
void ChordItem::renderInit(CuteReport::ScriptEngineInterface *scriptEngine, InitFlags flags)
{
...
Q_EXPORT_PLUGIN2(ChordItem, ChordItem)
#endif
