...
bool StorageResource::moduleInit()
{
    initMyResource();
    return true;
}
        ...
        if (filters.testFlag(QDir::Dirs)) {
            if (path.startsWith(prefix)) {
                ...
                if (relativePath.contains("/")) {
                    ...
                    StorageObjectInfo objectInfo;
                    objectInfo.name = filePathRoot;
                    objectInfo.url = urlScheme() + ":" + prefix + "/" + objectInfo.name;
...
CuteReport::StorageHelperInterface * StorageResource::createHelper(CuteDesigner::Core * designer)
{
//    if (!m_helper)
//        m_helper = new ResourceStorageHelper(this);
//    return m_helper;
    return new ResourceStorageHelper(this, designer);
}
