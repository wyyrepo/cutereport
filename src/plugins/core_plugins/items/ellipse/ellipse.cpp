    ...
    :ItemInterface(new EllipseItemPrivate, parent)
{
    ...
    d->geometry = QRectF(0,0,30,20);
    d->frame = 0;
}


bool EllipseItem::moduleInit()
{
    initMyResource();
    return true;
}
...
BaseItemHelperInterface *EllipseItem::createHelper(DesignerItemInterface *designer)
{
    Q_UNUSED(designer)
    return 0;
}
...
bool EllipseItem::renderBegin()
{
    rendererBeginBaseStart(new EllipseItemPrivate(*(reinterpret_cast<EllipseItemPrivate*>(d_ptr))));
...
RenderedItemInterface *EllipseItem::renderView(quint32 id)
{
    Q_D(EllipseItem);
...
void EllipseItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
    ...
    painter->setPen(penPrepared(d->pen, d->dpi));
...
void EllipseItem::renderInit(CuteReport::ScriptEngineInterface *scriptEngine, CuteReport::InitFlags flags)
{
...
Q_EXPORT_PLUGIN2(EllipseItem, EllipseItem)
#endif
