...
BaseItemHelperInterface *ArcItem::createHelper(DesignerItemInterface *designer)
{
    Q_UNUSED(designer)
    return 0;
}
...
void ArcItem::renderInit(ScriptEngineInterface *scriptEngine, InitFlags flags)
{
...
bool ArcItem::renderBegin()
{
    rendererBeginBaseStart(new ArcItemPrivate(*(reinterpret_cast<ArcItemPrivate*>(d_ptr))));
...
RenderedItemInterface *ArcItem::renderView(quint32 id)
{
    Q_D(ArcItem);
...
void ArcItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
...
Q_EXPORT_PLUGIN2(ArcItem, ArcItem)
#endif
