...
BaseItemHelperInterface *PieItem::createHelper()
{
    return 0;
}
...
bool PieItem::renderBegin()
{
    rendererBeginBaseStart(new PieItemPrivate(*(reinterpret_cast<PieItemPrivate*>(d_ptr))));
...
RenderedItemInterface *PieItem::renderView(quint32 id)
{
    Q_D(PieItem);
...
void PieItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
...
void PieItem::renderInit(ScriptEngineInterface *scriptEngine, InitFlags flags)
{
...
Q_EXPORT_PLUGIN2(PieItem, PieItem)
#endif
