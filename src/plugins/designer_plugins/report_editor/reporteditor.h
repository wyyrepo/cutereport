...
//    void slotRendererChanged(CuteReport::RendererInterface *renderer);
    ...
    void slotRequestForNewStorageName(const QString & storageName);

    void slotRequestForNewRenderer(const CuteReport::RendererInterface* renderer);
    void slotRequestForDeleteRenderer(const QString & name);
    void slotRequestForChangeDefaultRenderer(const QString & name);
    void slotRequestForNewRendererName(const QString & name);

    void slotRequestForNewPrinter(const CuteReport::PrinterInterface* printer);
    void slotRequestForDeletePrinter(const QString & name);
    void slotRequestForChangeDefaultPrinter(const QString & name);
    void slotRequestForNewPrinterName(const QString & name);

    void slotRequestToInheritReport();
    void slotRequestForDetachReport();
...
//    void slotRequestForRenameReport(int index);
    ...
    void slotAppIsAboutToClose(bool* cancel);
    void slotOpenPrevReport();

    void slotRendererStarted(CuteReport::ReportInterface*report);
    void slotRendererStopped(CuteReport::ReportInterface*report, bool);
...
private:
    ...
    void putReportNameOnTop(const QString & reportUrl);
    void saveOpenReportList();
...
private:
    ...
    int m_prevReportNumber;
    QStringList m_prevReports;
    ...
    QAction * m_closeReportAction;
    QAction * m_previousReportsAction;
};
