...
static const char * unitShortStr[] = {QT_TR_NOOP("mm"), QT_TR_NOOP("pt"), QT_TR_NOOP("in"), QT_TR_NOOP("pica"),  QT_TR_NOOP("didot"),
...
QRectF convertUnit(const QRectF & rect, Unit unitIn, Unit unitOut, int dpi, bool roundPixels)
{
    if (unitIn == unitOut)
        return rect;
    ...
    case Pixel:
        return QRectF( QPointF( roundPixels ? mm2pixel_ceil(rectMM.left(), dpi) : mm2pixel(rectMM.left(), dpi), roundPixels ? mm2pixel_ceil(rectMM.top(), dpi) : mm2pixel(rectMM.top(), dpi) ),
                       QPointF( roundPixels ? mm2pixel_ceil(rectMM.right(), dpi) : mm2pixel(rectMM.right(), dpi), roundPixels ? mm2pixel_ceil(rectMM.bottom(), dpi) : mm2pixel(rectMM.bottom(), dpi)) );
    default: return QRectF();
    }
...
QPointF convertUnit(const QPointF & point, Unit unitIn, Unit unitOut, int dpi, bool roundPixels)
{
    if (unitIn == unitOut)
        return point;
    ...
    case Pixel:
        return QPointF(roundPixels ? mm2pixel_ceil(pointMM.x(), dpi) : mm2pixel(pointMM.x(), dpi), roundPixels ? mm2pixel_ceil(pointMM.y(), dpi) : mm2pixel(pointMM.y(), dpi));
    default: return QPointF();
    }
...
QSizeF convertUnit(const QSizeF & size, Unit unitIn, Unit unitOut, int dpi, bool roundPixels)
{
    if (unitIn == unitOut)
        return size;
    ...
    case Pixel:
        return QSizeF(roundPixels ? mm2pixel_ceil(sizeMM.width(), dpi) : mm2pixel(sizeMM.width(), dpi), roundPixels ? mm2pixel_ceil(sizeMM.height(), dpi) : mm2pixel(sizeMM.height(), dpi));
    default:
        return QSizeF();
    }
...
qreal convertUnit(const qreal & value, Unit unitIn, Unit unitOut, int dpi, bool roundPixels)
{
    if (unitIn == unitOut)
        return value;
    ...
    case Pixel:
        return roundPixels ? mm2pixel_ceil(valueMM, dpi) : mm2pixel(valueMM, dpi);
    default:
        return 0;
    }
