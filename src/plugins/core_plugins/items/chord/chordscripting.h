...
{
    QScriptValue clazz = engine->globalObject().property(QString::fromLatin1("chord"));
...
static const char * const qtscript_ChordItem_function_names[] = {
    "chord"
    , "toString"
};


static QScriptValue qtscript_ChordItem_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    switch (_id) {
        case 0:
            return context->throwError(QScriptContext::TypeError,
                                       QString::fromLatin1("ChordItem cannot be constructed"));
            break;

        default:
            Q_ASSERT(false);
    }
    return qtscript_Qt_throw_ambiguity_error_helper(context,  qtscript_ChordItem_function_names[_id],  "" /*qtscript_Qt_function_signatures[_id]*/);
}


QScriptValue qtscript_create_ChordItem_class(QScriptEngine *engine)
{
    QScriptValue proto = QScriptValue();
    QScriptValue ctor = engine->newFunction(qtscript_ChordItem_static_call, proto, 0);
...
void registerChordItemScriptClass(QScriptEngine *engine)
{
    QScriptValue extensionObject = engine->globalObject();
    extensionObject.setProperty("chord", qtscript_create_ChordItem_class(engine), QScriptValue::SkipInEnumeration);
}
