...
public:
    ...
    virtual RenderedItemInterface * renderView(quint32 id);

    static void paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate *data, const QRectF &boundingRect, RenderingType type);
...
public:
    ...
    {
        Q_UNUSED(widget)
        DummyItem::paint(painter, option, ptr(), rect(), RenderingTemplate);
    }
};
...
public:
    ...
    {
        Q_UNUSED(widget)
        DummyItem::paint(painter, option, d_ptr, rect(), RenderingReport);
    }
