...
void DummyBand::renderInit(ScriptEngineInterface *scriptEngine, InitFlags flags)
{
    renderInitBase(scriptEngine, flags);
}


bool DummyBand::renderBegin()
{
    Q_D(DummyBand);
    rendererBeginBaseStart(new DummyBandPrivate(*d));
    rendererBeginBaseEnd();
    return true;
}


RenderedItemInterface *DummyBand::renderView(quint32 id)
{
    Q_D(DummyBand);
