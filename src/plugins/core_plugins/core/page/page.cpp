...
bool cmpPressedItems(BaseItemInterface * a, BaseItemInterface * b)
{
    int al = a->childLevel();
    int bl = b->childLevel();
    if (al == bl)
    ...
    m_currentProperty(-1)
{
    ...
    d->order = 0;
    d->printOnPreviousPage = false;
    d->resetPageNumber = false;
    d->pageFlags = 0;
...
Page::~Page()
{
    QList <BaseItemInterface*> itemList = items();
    QListIterator<BaseItemInterface*> it(itemList);
    while (it.hasNext()) {
        BaseItemInterface* item = it.next();
        item->aboutToBeDeleted();
    }
    delete m_gui;
...
PageHelperInterface *Page::createHelper(DesignerItemInterface *designer)
{
    Q_UNUSED(designer)
    return 0;
}
...
bool Page::printOnPreviousPage() const
{
    return d->printOnPreviousPage;
}


void Page::setPrintOnPreviousPage(bool value)
{
    if (d->printOnPreviousPage == value)
        return;
    d->printOnPreviousPage = value;
    emit printOnPreviousPageChanged(d->printOnPreviousPage);
    emit changed();
}


int Page::pageFlags() const
{
    return d->pageFlags;
}


void Page::setPageFlags(int flags, bool value)
{
    if (value)
        d->pageFlags |= flags;
    else
...
bool Page::isPageFlagSet(int flag)
{
    return d->pageFlags & flag;
}
...
bool Page::addItem(BaseItemInterface * item, QPointF pagePos, QString * error)
{
    ...
    if (!CuteReport::ReportCore::isNameUnique(item, item->objectName(), parent()))
        item->setObjectName( CuteReport::ReportCore::uniqueName(item, item->moduleShortName().toLower(), parent()));
...
void Page::_deleteItem(BaseItemInterface *item, bool emitSignals, bool directDeletion)
{
    ...
    // remove children first, since it can cause crash when parent removed first
    // crash on removing item that contains children
...
void Page::setMarginLeft(qreal margin, CuteReport::Unit unit)
{
    ...
    emit marginsChanged(margins(unit));
    emit changed();
}
...
{
    if (!m_inited) {
        return CuteReport::Margins(d->marginLeft, d->marginTop, d->marginRight, d->marginBottom);
    }
    CuteReport::Unit u = (unit == UnitNotDefined) ? d->unit : unit;
    qreal left = convertUnit(d->marginLeft, Millimeter, u, d->dpi);
    qreal top = convertUnit(d->marginRight, Millimeter, u, d->dpi);
    qreal right = convertUnit(d->marginTop, Millimeter, u, d->dpi);
    qreal bottom = convertUnit(d->marginBottom, Millimeter, u, d->dpi);
    return CuteReport::Margins(left, top, right, bottom);
}


void Page::setMargins(const CuteReport::Margins &margins, CuteReport::Unit unit)
{
    _setMargin(d->marginLeft, margins.left(), unit, false);
    _setMargin(d->marginRight, margins.right(), unit, false);
    _setMargin(d->marginTop, margins.top(), unit, false);
    _setMargin(d->marginBottom, margins.bottom(), unit, false);
    afterGeometryChanged();
    emit marginsChanged(this->margins(unit));
    emit changed();
}
...
BaseItemInterface * Page::itemAt(QPointF pos)
{
    ...
    QList<BaseItemInterface*> children = findChildren<BaseItemInterface *>();
    ...
    qSort(items.begin(), items.end(), cmpPressedItems);
    return items.first();
}
...
void Page::renderInit(ScriptEngineInterface *scriptEngine)
{
    d->_m_dpi = d->dpi;
...
CuteReport::RenderedPageInterface * Page::render(int /*customDPI*/)
{
    if (d->resetPageNumber)
        m_renderer->setCurrentPageNumber(1);
...
bool Page::getResetPageNumber() const
{
    return d->resetPageNumber;
}


void Page::setResetPageNumber(bool value)
{
    if (d->resetPageNumber == value)
        return;
    d->resetPageNumber = value;
    emit resetPageNumberChanged(d->resetPageNumber);
    emit changed();
}
