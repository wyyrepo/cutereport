...
{
    QScriptValue clazz = engine->globalObject().property(QString::fromLatin1("Arc"));
...
static const char * const qtscript_ArcItem_function_names[] = {
    "Arc"
    , "toString"
};


static QScriptValue qtscript_ArcItem_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    switch (_id) {
        case 0:
            return context->throwError(QScriptContext::TypeError,
                                       QString::fromLatin1("ArcItem cannot be constructed"));
            break;

        default:
            Q_ASSERT(false);
    }
    return qtscript_Qt_throw_ambiguity_error_helper(context,  qtscript_ArcItem_function_names[_id],  "" /*qtscript_Qt_function_signatures[_id]*/);
}


QScriptValue qtscript_create_ArcItem_class(QScriptEngine *engine)
{
    QScriptValue proto = QScriptValue();
    QScriptValue ctor = engine->newFunction(qtscript_ArcItem_static_call, proto, 0);
...
void registerArcItemScriptClass(QScriptEngine *engine)
{
    QScriptValue extensionObject = engine->globalObject();
    extensionObject.setProperty("Arc", qtscript_create_ArcItem_class(engine), QScriptValue::SkipInEnumeration);
}
