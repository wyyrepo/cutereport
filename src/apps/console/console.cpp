...
static const char* help = "Usage: cutereport -i filename[,filename]...\n\n"
                          "  -g, --gui                     run in GUI mode (template designer) \n"
                          "  -gm, --gui-module-switch <modulename> \n"
                          "                                switch to the specified module page \n"
                          "  -gs                           QUI/Qt styles \n"
                          ...
                          "  -ps --preview-size            sets size of preview window in format \'width x height\',\n"
                          "                                   example: -ps 640x480\n"
        ...
        switch (m_output) {
            ...
            case Preview: {
                    ...
                    if (report) {
                        ...
                        int width = 800;
                        int height = 600;
                        if (!m_previewSize.isEmpty()) {
                            int w = m_previewSize.section(QRegExp("[Xx]"),0,0).toInt();
                            int h = m_previewSize.section(QRegExp("[Xx]"),1,1).toInt();
                            if (w > 0) width = w;
                            if (h > 0) height = h;
                        }
...
{
    QString initConfigFile = QCoreApplication::applicationDirPath() + "/init.cfg";
    QSharedPointer<QSettings> initSettings;
    if (QFile::exists(initConfigFile)) {
        initSettings = QSharedPointer<QSettings>(new QSettings(initConfigFile, QSettings::IniFormat, this));
    }
    QString headersPath = initSettings.isNull() || initSettings->value("HeadersPath").isNull()
                          ? REPORT_HEADERS_PATH : initSettings->value("HeadersPath").toString();
    QString libsPath = initSettings.isNull() || initSettings->value("LibsPath").isNull()
                       ? REPORT_LIBS_PATH : initSettings->value("LibsPath").toString();
    QString priFilePath = initSettings.isNull() || initSettings->value("PriFilePath").isNull()
                          ? REPORT_HEADERS_PATH : initSettings->value("PriFilePath").toString();
    QString headersPrefix = initSettings.isNull() || initSettings->value("HeadersPrefix").isNull()
                                   ? QString() : initSettings->value("HeadersPrefix").toString();

    QDir dir(headersPath);
    ...
    if (headerFile.open(QFile::WriteOnly | QFile::Truncate)) {
        ...
        out << "#ifndef CUTEREPORT_INCLUDES_H \n";
        out << "#define CUTEREPORT_INCLUDES_H \n";
        out << "\n";
        foreach (QString fileName, files) {
            out << QString("#include \"%1%2\"\n").arg(headersPrefix, fileName);
        }
        out << "\n";
        out << "#endif";
    }
    ...
    QDir priDir(priFilePath);
    QFile priFile;
    if (priDir.isRelative())
        priFile.setFileName( QCoreApplication::applicationDirPath() + "/" + priDir.path() + "/CuteReport.pri");
    else
        priFile.setFileName(priDir.absolutePath() + "/CuteReport.pri");
    ...
    if (priFile.open(QFile::WriteOnly | QFile::Truncate | QFile::Text)) {
        ...
        libPath.setPath(headersPath);
        out << QString("INCLUDEPATH += \"%1\" \n").arg(libPath.absolutePath());

        libPath.setPath(libsPath);
        out << QString("LIBS += -L\"%1\" \n").arg(libPath.absolutePath().isEmpty() ? QDir::currentPath() : libPath.absolutePath());

        out << "win32: CONFIG(debug, debug|release): LIBS += -lCuteReportCored -lCuteReportWidgetsd \n";
        out << "else: LIBS += -lCuteReportCore -lCuteReportWidgets \n";
    }

    m_reportCore = new CuteReport::ReportCore(0 ,0, false, !m_logFile.isEmpty());
    m_reportCore->settings()->setValue("CuteReport/AppPath",  QCoreApplication::applicationDirPath());
    delete m_reportCore;
}


void Console::reportInstallPaths()
{
...
void Console::parseArguments(int argc, char *argv[])
{
    ...
    while (i<argc) {
        ...
        }  else if (arg == "--preview-size" || arg == "-ps") {
            if (nextArg.startsWith("-")) {
                std::cout << QString("Error: Renderer options is not correct: \'%1\'\n").arg(nextArg).toStdString();
                exit(1);
            }
            m_previewSize = nextArg;
        } else if (arg == "--paths") {
            reportInstallPaths();
            exit(0);
        } else if (arg == "-gs") {
            std::cout << QStyleFactory::keys().join(", ").append("\n").toStdString();
            exit(0);
        }
