    ...
    : d_ptr(itemPrivateData)/*, m_item(item)*/
{
    ...
    if (item) {
        ...
        d_ptr->geometry.translate(absPos);
    }
...
{
    QRectF itemPixelRect = convertUnit(d_ptr->geometry, Millimeter, Pixel, d_ptr->dpi);
...
{
    Q_UNUSED(widget);
    ItemInterface::paintBegin(painter, option, d_ptr, rect(), RenderingReport);
}
...
{
    Q_UNUSED(widget);
    ItemInterface::paintEnd(painter, option, d_ptr, rect(), RenderingReport);
}
...
QRectF RenderedItemInterface::absoluteGeometry(Unit unit) const
{
    ...
    return convertUnit(d_ptr->geometry, Millimeter, u, d_ptr->dpi);
...
void RenderedItemInterface::setAbsoluteGeometry(const QRectF & geometry, Unit unit)
{
    if (!d_ptr)
        return;
    Unit u = (unit == UnitNotDefined) ? d_ptr->unit : unit;
    d_ptr->geometry = convertUnit(geometry, u, Millimeter, d_ptr->dpi);
}
...
{
    QGraphicsItem * parent = parentItem();
    QPointF itemPixelpos = convertUnit(d_ptr->geometry.topLeft(), d_ptr->unit, Pixel, d_ptr->dpi);
...
QRectF RenderedItemInterface::absoluteBoundingRect(Unit unit) const
{
    ...
    int delta = d_ptr->geometry.height() * sin(d_ptr->rotation * 3.14159265 /180);
    trans.translate(delta,0);

    QRectF rect = QRectF(QPointF(0,0), QSizeF(d_ptr->geometry.size()));
    QRectF resRect = QRectF(d_ptr->geometry.topLeft(), trans.mapRect(rect).size());
