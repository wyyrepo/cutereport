...
class DESIGNER_EXPORTS ModuleInterface : public QObject
{
    Q_OBJECT
public:
    enum ModuleRunType {RunNotDefined, RunForeground, RunOnceOnStart, RunOnDemand};
    ...
    virtual void stdActionTriggered(CuteDesigner::Core::StdAction actionType, QAction * action) {Q_UNUSED(actionType); Q_UNUSED(action)}

    QSet<QString> reportVariables() {return QSet<QString>();}

    virtual ModuleRunType runType() {return RunNotDefined;}
