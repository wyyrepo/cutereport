...
Log::~Log()
{
    logMessage(LogDebug, "Log", "DTOR", "");

    stopCustomHandler();
//    delete m_mouseHandler;
    m_logProcessor->moveToThread(this->thread());
...
void Log::init()
{
    ...
    m_interceptMouseEvents = m_settings ? m_settings->value("CuteReport_Log/mouseEvents", false).toBool() : false;
    m_interceptQtMessages = m_settings ? m_settings->value("CuteReport_Log/messagesQt", true).toBool() : false;
    m_logRotateCounter = m_settings ? m_settings->value("CuteReport_Log/logRotateCounter", 5).toLongLong() : 5;
    QString logRotateFileSize = m_settings ? m_settings->value("CuteReport_Log/logRotateFileSize", "50mb").toString() : "50mb";
    ...
    m_sync = m_settings ? m_settings->value("CuteReport_Log/synchronously", true).toBool() : true;
    m_enabled = m_settings ? m_settings->value("CuteReport_Log/enabled", true).toBool() : true;
    ...
    if (m_enabled) {
        QString context_size = m_settings ? m_settings->value("CuteReport_Log/contextMaxSize", "1kb").toString() : "1kb";
        int s = 0;
        ...
        m_logProcessor->setMouseEventFile(m_settings ? m_settings->value("CuteReport_Log/mouseLogFilename", "").toString() : "");
...
void Log::log_stderr(const QString & message)
{
//    std::cerr <<  message.toStdString() << std::endl;
    if (m_log)
        m_log->m_cache << message;
}
...
void Log::setDefaultConfig()
{
    ...
    if (m_settings->value("CuteReport_Log/logFileTimestampFor").toString().isEmpty())
        m_settings->setValue("CuteReport_Log/logFileTimestampFor","debug|warning|error|critical|info|metric");

    if (m_settings->value("CuteReport_Log/debugTo").toString().isEmpty())
        m_settings->setValue("CuteReport_Log/debugTo","console|file");

    if (m_settings->value("CuteReport_Log/warningTo").toString().isEmpty())
        m_settings->setValue("CuteReport_Log/warningTo","console|file");

    if (m_settings->value("CuteReport_Log/errorTo").toString().isEmpty())
        m_settings->setValue("CuteReport_Log/errorTo","console|file");

    if (m_settings->value("CuteReport_Log/criticalTo").toString().isEmpty())
        m_settings->setValue("CuteReport_Log/criticalTo","console|file");
}
...
Log::Destination Log::settingLineToDestination(QString level, QString line, bool listExists, QStringList allowedDestinations )
{
    ...
    if (m_settings) {
        ...
        dest.needContext = m_settings->value("CuteReport_Log/contextFor").toString().contains(level, Qt::CaseInsensitive) ? true : false;
        dest.needLogfileTimestamp = m_settings->value("CuteReport_Log/logFileTimestampFor").toString().contains(level, Qt::CaseInsensitive) ? true : false;
...
Log::Module Log::getGeneralLevelsSettings()
{
    ...
    for (int i = 1; i<LOGLEVELMAXVALUE; ++i) {
        ...
        QString param = QString("CuteReport_Log/%1To").arg(LogLevelNames.value(LogLevel(i)));
...
QHash<QString, Log::Module> Log::getModulesLevelsSettings()
{
    ...
    foreach (QString moduleName, modulesNames) {
        ...
        if (listExists) {
            allowedDestinations = m_settings->value(QString("CuteReport_Log/moduleDest_%1")
...
//            module.levelsDefined = true;
            QStringList moduleLevels = m_settings->value(QString("CuteReport_Log/moduleLevels_%1")
                                                          .arg(moduleName)).toString()
                                                          .split("|", QString::SkipEmptyParts);
        ...
        for (int i=1; i<LOGLEVELMAXVALUE; ++i) {
            ...
            if (keys.contains(levelKey)) {
                QString destForCurrentLevel = QString("CuteReport_Log/"+levelKey);
...
void Log::logRotate(QString filename)
{
    ...
    QCoreApplication::setOrganizationName("ExaroLogic");
    QCoreApplication::setApplicationName("CuteReport");
    #if QT_VERSION >= 0x050000
        QString logFileName = QStandardPaths::writableLocation(QStandardPaths::DataLocation) + "/" + LOGFILENAME;
    #else
        QString logFileName = QDesktopServices::storageLocation(QDesktopServices::DataLocation) + "/" + LOGFILENAME;
    #endif
    QCoreApplication::setOrganizationName(appName);
    QCoreApplication::setApplicationName(orgName);
#else
   QString logFileName = filename.isEmpty() ? LOGFILENAME : filename;
#endif

   QFile log(logFileName);
    if (filename.isEmpty()) {     // default file - first iteration
...
void Log::flashCache()
{
    ...
    foreach(const QString & msg, m_cache)
        m_log->push(LogDebug, "Log", msg);
}
...
//            qDebug() << key;
            QString regExp = m_settings->value("CuteReport_Log/"+key).toString();
            QString replace = m_settings->value(QString("CuteReport_Log/filterAction_%1").arg(filterName)).toString();
            f.re = QRegExp(regExp);
            f.replace = replace;
            filters.append(f);
        }
