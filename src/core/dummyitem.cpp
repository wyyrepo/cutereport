...
BaseItemHelperInterface *DummyItem::createHelper()
{
    return 0;
}
...
void DummyItem::renderInit(ScriptEngineInterface *scriptEngine, CuteReport::InitFlags flags)
{
    renderInitBase(scriptEngine, flags);
}


bool DummyItem::renderBegin()
{
    Q_D(DummyItem);
    rendererBeginBaseStart(new DummyItemPrivate(*d));
    rendererBeginBaseEnd();
    return true;
}


RenderedItemInterface *DummyItem::renderView(quint32 id)
{
    Q_D(DummyItem);
...
void DummyItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
