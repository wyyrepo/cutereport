...
SqlDatasetHelper::SqlDatasetHelper(SqlDataset *dataset, CuteDesigner::Core * designer)
...
//    SQLSyntaxHighlighter * syntax = new SQLSyntaxHighlighter(m_ui->textEdit->document());
    ...
    connect(m_dataset, SIGNAL(optionsChanged(QString)), this, SLOT(slotOptionsChanged(QString)));
...
void SqlDatasetHelper::save()
{
    ...
    m_dataset->setOptions(m_ui->optionsField->text());
}
...
void SqlDatasetHelper::load()
{
    ...
    m_ui->optionsField->setText(m_dataset->options());
}
...
void SqlDatasetHelper::slotOptionsChanged(QString options)
{
    if (options != m_ui->optionsField->text())
        m_ui->optionsField->setText(options);
}
...
////    d.setUrlHint(m_dataset->dbname());
//    if (!d.exec())
//        return;

//    QString resultUrl = d.currentObjectUrl();

    QString resultUrl = m_designer->loadObjectDialog(QStringList(), tr("Open Database file"));
