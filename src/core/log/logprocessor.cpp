  ...
  ,m_useFilters(false)
{
    ...
    QCoreApplication::setApplicationName("CuteReport");
        ...
        m_logFileName = QStandardPaths::writableLocation(QStandardPaths::DataLocation) + "/" + LOGFILENAME;
    #else
        m_logFileName = QDesktopServices::storageLocation(QDesktopServices::DataLocation) + "/" + LOGFILENAME;
    #endif
...
//        QString module = _submodule.isEmpty() ? _module : _module + "::" + _submodule;
        ...
        if (_fullMessage.isEmpty())
            log_stderr(QTime::currentTime().toString("HH:mm:ss  ") + sender + _message  );
        else
            log_stderr(QTime::currentTime().toString("HH:mm:ss  ") + sender + _message + " ^ " + _fullMessage);
    }
    ...
    if (dest.file) {
        ...
        if (log.open(QFile::WriteOnly| QFile::Append)) {
            ...
            if (_fullMessage.isEmpty())
                ...
                out << QString("%1    %2 %3 ^ %4").arg(date).arg(sender).arg(_message).arg(_fullMessage) << '\n';
            log.close();
        }
    }
}
