...
PropertyInterface::PropertyInterface(QObject *parent , QObject * object, int property, const PropertyModel * pm): QObject(parent),m_propertyModel(pm)
{
    ...
    if (object) {
        ...
        QMetaProperty prop = metaObject->property(property);
        if (prop.hasNotifySignal()) {
            QMetaMethod signal = prop.notifySignal();
            QMetaMethod updateSlot = this->metaObject()->method(this->metaObject()->indexOfSlot("propertyChangedOutside()"));
#if QT_VERSION < 0x040800
            QString signalStr = QString("2%1").arg(signal.signature());
            QString slotStr = QString("1%1").arg(updateSlot.signature());
            connect(object, signalStr.toLatin1().constData(), this, slotStr.toLatin1().constData());
#else
            connect(object, signal, this, updateSlot);
#endif
        }
    }
}
...
{
    QMetaProperty prop = object()->metaObject()->property(objectProperty());
...
{
    QMetaProperty prop = object()->metaObject()->property(objectProperty());
    QVariant old_data = prop.read(object());
    bool ret = prop.write(object(), data);

    if (ret)
        emit(propertyChanged(object(), QString(prop.name()), old_data, data));
    return ret;
}
...
QModelIndex PropertyInterface::index() const
{
    return m_index;
}

void PropertyInterface::setIndex(const QModelIndex &index)
{
    m_index = index;
}
...
void PropertyInterface::propertyChangedOutside()
{
    const PropertyModel * model = reinterpret_cast<const PropertyModel *>(m_propertyModel);
    if (!model)
        return;
    model->emitDataChangedSignal(m_index, QModelIndex());
}
