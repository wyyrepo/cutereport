...
void ReportPreview::initMe(PreviewFlags f)
{
    ...
    connect(ui->buttonEdit, SIGNAL(toggled(bool)), this, SLOT(slotEdit(bool)));
    ...
    connect(&m_progressDialogDelayTimer, SIGNAL(timeout()), this, SLOT(doShowProgressDialog()));
...
void ReportPreview::connectCore()
{
    if (m_reportCore && !m_reportCoreInited) {
        ...
        connect(m_reportCore, SIGNAL(rendererProcessingPage(CuteReport::ReportInterface*,int,int, int, int)), this, SLOT(slotRendererProcessingPage(CuteReport::ReportInterface*,int,int,int,int)));
...
void ReportPreview::setFitMode(ReportPreview::ResultFitMode fitMode)
{
    m_fitMode = fitMode;
}
...
qreal ReportPreview::viewScale() const
{
    switch (m_zoomStyle) {
    case ZoomDPI:
        return m_viewDpi;
...
void ReportPreview::setViewScale(qreal scale)
{
...
void ReportPreview::slotExport()
{
    ...
    ExportInterface * exportObject = m_reportCore->createExportObject(moduleName);
    if (!exportObject)
        return;
    ...
    exportObject->processWithDialog(m_report, d.currentObjectUrl());
    emit exported();
...
void ReportPreview::slotEdit(bool value)
{
    if (value) {
        m_editor = new ReportPreviewEditor(m_scene, this);
...
void ReportPreview::showProgressDialog(const QString & labelText, const QString & cancelButtonText, int minimum, int maximum)
{
    ...
    m_progressDialogDelayTimer.setSingleShot(true);
    m_progressDialogDelayTimer.start(750);
}
...
void ReportPreview::doShowProgressDialog()
{
    if (m_progressDialog) {
        delete m_progressDialog;
        m_progressDialog = 0;
    ...
    /// checking for report's runtime forms
    bool cuteReportFormOpen = false;
    QWidgetList list = QApplication::topLevelWidgets ();
    foreach (QWidget * w, list) {
        if (w->property("_cutereport_form").toBool() == true) {
            cuteReportFormOpen = true;
            break;
        }
    }

    if (cuteReportFormOpen) {
        m_progressDialogDelayTimer.setSingleShot(true);
        m_progressDialogDelayTimer.start(750);
        return;
    }

    m_progressDialog = new QProgressDialog("", "Cancel", 0, 100, m_reportCore->rootWidget() ? m_reportCore->rootWidget() : this);
    m_progressDialog->setMinimumDuration(0);
    m_progressDialog->setMinimumWidth(300);
    m_progressDialog->setWindowTitle("CuteReport Rendering...");
...
void ReportPreview::slotRendererDone(CuteReport::ReportInterface *report, bool)
{
    ...
    if (m_fitMode == FitPage)
        slotFitPage();
    else if (m_fitMode == FitWidth)
        slotFitWidth();
}


void ReportPreview::slotRendererProcessingPage(CuteReport::ReportInterface *report,int value, int total, int pass, int passTotal)
{
    if (m_report && report != m_report)
        return;

    QString passInfo = pass > 1 ? QString("(pass %1 of %2)").arg(pass).arg(passTotal) : QString();
    int maxValue = total; // TODO: correct taking pass/passes into account

    if (total > 0)
        updateProgressDialog(QString("Processing page %1 of %2 %3").arg(value).arg(total).arg(passInfo), value, maxValue);
    else
        updateProgressDialog(QString("Processing page %1 %2").arg(value).arg(passInfo), value, maxValue);
}
...
void ReportPreview::clearEditor()
{
    if (!m_editor)
        return;

    ui->view->setMouseTracking(false);
    m_scene->removeEditor();
    delete m_editor;
    m_editor = 0;

    ui->buttonEdit->blockSignals(true);
    ui->buttonEdit->setChecked(false);
    ui->buttonEdit->blockSignals(false);
}
...
//    page->setShowFlags(CuteReport::RenderedPageInterface::ShowDefaultPreview);
...
bool ReportPreview::showProgress() const
{
    return m_showProgress;
}


void ReportPreview::setShowProgress(bool showProgress)
{
...
void GraphicsScene::mouseMoveEvent ( QGraphicsSceneMouseEvent * mouseEvent )
{
    ...
    m_editor->sceneMouseMoveEvent(mouseEvent);
...
void GraphicsScene::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    ...
    m_editor->mouseDoubleClickEvent(mouseEvent);
