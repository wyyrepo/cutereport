...
CuteReport::StorageSyncStatus StorageGit::sync()
{
    if (!isBinaryCorrect && !(isBinaryCorrect = SyncManager::checkBinary(m_gitBinaryPath))) {
        emit syncMessage("Error: GIT binary path is not correct!");
//        if (m_helper)
...
//        m_helper->showSyncMessage(m_syncStruct->manager.output() + m_syncStruct->manager.stdOutput() + m_syncStruct->manager.stdError());
    emit syncMessage(m_syncStruct->manager.output() + m_syncStruct->manager.stdOutput() + m_syncStruct->manager.stdError());
                                             ...
                                             QDir::Filters filters, QDir::SortFlags sort, bool * ok)
{
    ...
    foreach (const QFileInfo &fileInfo, infoList) {
        ...
        objectInfo.name = pathCutOff(fileInfo.absoluteFilePath());
        objectInfo.url = urlScheme() + ":" + objectInfo.name;
...
CuteReport::StorageHelperInterface * StorageGit::createHelper(CuteDesigner::Core * designer)
{
//    if (!m_helper)
//        m_helper = new GitStorageHelper(this);
//    return m_helper;
    return new GitStorageHelper(this, designer);
}
