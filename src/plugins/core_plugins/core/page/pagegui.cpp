  ...
  //    m_zoom(1.0)
{
    ...
    connect(m_page, SIGNAL(marginsChanged(CuteReport::Margins)), this, SLOT(slotMarginChanged()));
...
void PageGUI::slotMouseDoubleClicked(QGraphicsSceneMouseEvent * event)
{
    ...
    if (!item) {
        emit m_page->pageHelperRequest();
        return;
    }

    emit m_page->itemHelperRequest(item);
//    BaseItemHelperInterface * helper = item->createHelper();
