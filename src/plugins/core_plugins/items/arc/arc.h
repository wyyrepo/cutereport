...
class ArcItemPrivate;

class  ArcItem : public CuteReport::ItemInterface
{
	Q_OBJECT
...
public:
    ...
    virtual CuteReport::BaseItemHelperInterface * createHelper(CuteReport::DesignerItemInterface * designer);
    ...
    static void paint(QPainter * painter, const QStyleOptionGraphicsItem *option, CuteReport::BaseItemInterfacePrivate *data, const QRectF &boundingRect, CuteReport::RenderingType type = CuteReport::RenderingTemplate);
...
class  ArcItemView : public CuteReport::ItemInterfaceView
{
    Q_INTERFACES(CuteReport::ItemInterfaceView)
public:
    ArcItemView(CuteReport::BaseItemInterface * item):
        CuteReport::ItemInterfaceView(item){}

    virtual void paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget = 0)
    {
        Q_UNUSED(widget)
        ArcItem::paint(painter, option, ptr(), boundingRect(), CuteReport::RenderingTemplate);
    }
};


class  RenderedArcItem : public CuteReport::RenderedItemInterface
{
public:
    explicit RenderedArcItem(CuteReport::BaseItemInterface * item, CuteReport::BaseItemInterfacePrivate *itemPrivateData)
        :RenderedItemInterface(item, itemPrivateData) {}

    virtual void paint(QPainter * painter, const QStyleOptionGraphicsItem *option, QWidget *widget = 0)
    {
        Q_UNUSED(widget)
        ArcItem::paint(painter, option, d_ptr, boundingRect(), CuteReport::RenderingReport);
    }

    virtual bool setEditMode(QWidget * topWidget) {return false;}
};
