...
{
    QScriptValue clazz = engine->globalObject().property(QString::fromLatin1("Pie"));
...
static const char * const qtscript_PieItem_function_names[] = {
    "Pie"
    , "toString"
};


static QScriptValue qtscript_PieItem_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    switch (_id) {
        case 0:
            return context->throwError(QScriptContext::TypeError,
                                       QString::fromLatin1("PieItem cannot be constructed"));
            break;

        default:
            Q_ASSERT(false);
    }
    return qtscript_Qt_throw_ambiguity_error_helper(context,  qtscript_PieItem_function_names[_id],  "" /*qtscript_Qt_function_signatures[_id]*/);
}


QScriptValue qtscript_create_PieItem_class(QScriptEngine *engine)
{
    QScriptValue proto = QScriptValue();
    QScriptValue ctor = engine->newFunction(qtscript_PieItem_static_call, proto, 0);
...
void registerPieItemScriptClass(QScriptEngine *engine)
{
    QScriptValue extensionObject = engine->globalObject();
    extensionObject.setProperty("Pie", qtscript_create_PieItem_class(engine), QScriptValue::SkipInEnumeration);
}
