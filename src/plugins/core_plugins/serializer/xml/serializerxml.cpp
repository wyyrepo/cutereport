...
QDomElement SerializerXML::objectProperties(const QObject * object, QDomDocument * doc, const QString & objectName)
{
    ...
    if (objectTypeName.isEmpty()) {
        ...
        } else if (dynamic_cast<const CuteReport::ItemExternalDataInterface*>(object)) {
            const CuteReport::ItemExternalDataInterface* dataObject = reinterpret_cast<const CuteReport::ItemExternalDataInterface*>(object);
            d.setAttribute("type", dataObject->typeName());
            d.setAttribute("name", dataObject->objectName());
        }
    ...
    if (object->children().size()) {
        ...
        for (int c = 0; c < object->children().size(); c++) {
            QDomElement el = objectProperties(object->children()[c], doc);
            if (!el.isNull())
                ch.appendChild(objectProperties(object->children()[c], doc));
        }

        if (ch.childNodes().count())
            d.appendChild(ch);
    }
...
QObject * SerializerXML::objectFromDom( QObject * parent, const QDomElement & dom, ReportInterface * report)
{
    ...
    if (obj) {
        ...
        /// some modules might save theirs objects as a group with parent of QObject
...
QDomElement SerializerXML::variantToDom(QDomDocument * doc, const QString & name, const QVariant & value)
{
    ...
    if (QString("QPainterPath") == typeName) {
        ...
        return dom;
    } else if (QString("QMargins") == typeName) {
        ...
        switch (value.type()) {
            ...
            */
                ...
                dom.appendChild(variantToDom(doc, "width", value.value<QPen>().widthF()));
                break;

            case QVariant::List:
                /*	<tag_name type="QVariantList">
                    <item>value</item>
                    <item>value</item>
                    ...
                    <item>value</item>
            ...
            */
                foreach(const QVariant & itemVal, value.toList()) {
                    dom.appendChild(variantToDom(doc, "item", itemVal));
                }
                break;
            ...
            */
                    ...
                    // sorting prevents unnecessary file diff
                    QStringList sortedKeys = value.toMap().uniqueKeys();
                    qSort(sortedKeys);
                    foreach(const QString &key, sortedKeys) {
                        QDomElement keyElement = doc->createElement("key");
                        keyElement.setAttribute("name", key);
                        QVariantList values = value.toMap().values(key);
        ...
        */
                    // alex: doesnt work if key contains slash
                    // sorting prevents unnecessary file diff
                    QStringList sortedKeys = value.toHash().uniqueKeys();
                    qSort(sortedKeys);
                    foreach(const QString &key, sortedKeys) {
                        QDomElement keyElement = doc->createElement("key");
                        keyElement.setAttribute("name", key);
                        QVariantList values = value.toHash().values(key);
                        foreach(const QVariant & val, values)
                            keyElement.appendChild(variantToDom(doc, "value", val));
                        dom.appendChild(keyElement);
                    }
...
QVariant SerializerXML::domToVariant(const QDomElement & dom)
{
    ...
    if (QString("QMargins") == dom.attribute("type", "").toLatin1()) {
        QStringList m = dom.text().split(',');
        if (m.size() == 4)
            value.setValue(QMargins(m[0].toDouble(), m[1].toDouble(), m[2].toDouble(), m[3].toDouble()));
        return value;
    }
    ...
    switch (QVariant::nameToType(dom.attribute("type", "").toLatin1()))
    {
        ...
        case QVariant::Pen:
            ...
            */
            ...
            pen.setWidthF(domToVariant(dom.firstChildElement("width")).toDouble());
            value = pen;
            break;
        ...
        case QVariant::List: {
                ...
                */
                QVariantList list;
                QDomElement val = dom.firstChildElement();
                for (;!val.isNull();val = val.nextSiblingElement())
                    list << domToVariant(val);
                value = list;
            }
            break;
...
QDomElement SerializerXML::propertyToDom(QDomDocument *doc, const QObject *object, int propertyNum)
{
    ...
    // workarounds
    if (dynamic_cast<const ReportInterface*>(object) && metaPropertyName == "objectName") // we have another property: "name"
        return element;
...
QObject * SerializerXML::createObject( const QDomElement & dom, QObject * parent, CuteReport::ReportInterface *report) const
{
    ...
    if (!moduleName.isEmpty() && !moduleName.contains("::"))
    ...
    } else if (tagName == "ExternalData") {
        BaseItemInterface * parentItem = dynamic_cast<CuteReport::BaseItemInterface*>(parent);
        if (parentItem && !dom.attribute("type").isEmpty() ) {
            object = parentItem->createDataObjectByTypeName(dom.attribute("type"));
            if (object)
                object->setObjectName(dom.attribute("name"));
        }
    }
...
QString SerializerXML::objectType(const QObject * object, QList<StringPair> &specialAttributes) const
{
    ...
    if (dynamic_cast<const CuteReport::FormInterface*>(object))
        return "Form";
    if (dynamic_cast<const CuteReport::ItemExternalDataInterface*>(object))
        return "ExternalData";
