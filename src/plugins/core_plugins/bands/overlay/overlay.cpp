...
void Overlay::renderInit(ScriptEngineInterface *scriptEngine, InitFlags flags)
{
    renderInitBase(scriptEngine, flags);
}


bool Overlay::renderNewPage()
{
    rendererBeginBaseStart(new OverlayPrivate(*(reinterpret_cast<OverlayPrivate*>(d_ptr))));
...
RenderedItemInterface *Overlay::renderView(quint32 id)
{
    Q_D(Overlay);
