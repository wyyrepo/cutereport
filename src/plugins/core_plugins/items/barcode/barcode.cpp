...
BaseItemHelperInterface *BarcodeItem::createHelper()
{
    return 0;
}
...
void BarcodeItem::renderInit(ScriptEngineInterface *scriptEngine, CuteReport::InitFlags flags)
{
    Q_D(const BarcodeItem);
    ...
    if (flags.testFlag(CuteReport::InitData)) {
        m_renderer->registerStrings(renderingStrings(), this);
    }
...
bool BarcodeItem::renderBegin()
{
    rendererBeginBaseStart(new BarcodeItemPrivate(*(reinterpret_cast<BarcodeItemPrivate*>(d_ptr))));

    Q_D(BarcodeItem);

    d->script = m_renderer->processString(CuteReport::ScriptString(reportObject(), this, d->script, "[", "]"));
    d->primaryMessage = m_renderer->processString(CuteReport::ScriptString(reportObject(), this, d->primaryMessage, "[", "]"));
...
RenderedItemInterface *BarcodeItem::renderView(quint32 id)
{
    Q_D(BarcodeItem);
...
void BarcodeItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    Q_UNUSED(option)
    Q_UNUSED(type)
...
QList<InternalStringData> BarcodeItem::renderingStrings()
{
    Q_D(BarcodeItem);
    QList<InternalStringData> list;
    list << InternalStringData(0, d->script, false, StringHasNoExpression);
    list << InternalStringData(1, d->primaryMessage, false, StringHasNoExpression);
    return list;
}
...
Q_EXPORT_PLUGIN2(BarcodeItem, BarcodeItem)
#endif
